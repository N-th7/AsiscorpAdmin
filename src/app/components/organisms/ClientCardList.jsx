...
const handleChange = (id, field, value) => {
  ...
  debounceRefs.current[id] = setTimeout(async () => {
    try {
          ...
          try {
            ...
            setCards((cardsAfterUpdate) =>
              cardsAfterUpdate.map((card) => {
                ...
                if (response?.data?.title) updated.title = response.data.title;
                if (response?.data?.text) updated.text = response.data.text;
                if (response?.data?.image && response.data.image !== card.image) {
                  updated.image = response.data.image;
                  updated.imagePreview = `${response.data.image}?t=${Date.now()}`;
                }
      ...
      console.error(`❌ Error al preparar actualización de cliente ${id}:`, error);
